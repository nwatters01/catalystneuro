"""Extract mean waveforms for V-probe.
...
def _run_session(session, probe_name, kilosort_run_name):
    ...
    om2_raw_data_path = os.path.join(om2_session_dir, 'raw_data', probe_name)
    print('om2_raw_data_path: {}'.format(om2_raw_data_path))
    if not os.path.exists(om2_raw_data_path):
        os.makedirs(om2_raw_data_path)

    # Look for raw data file in om2
    raw_data_path = os.listdir(om2_raw_data_path)
    if len(raw_data_path) == 0:
        ...
        raw_data_path = os.path.join(
            om4_session_dir, 'raw_data', probe_name, 'raw_data.dat')
        print('raw_data_path: {}'.format(raw_data_path))
        if not os.path.exists(raw_data_path):
            print('Found no raw data, exiting')
            return
        bash_command = (
            'cp {} {}/'.format(raw_data_path, om2_raw_data_path)
        )
        print('Running bash command {}'.format(bash_command))
        os.system(bash_command)

    raw_data_file = os.path.join(om2_raw_data_path, 'raw_data.dat')
...
def main(monkey):
    ...
    kilosort_run_name = 'ks_3_output_pre_v6_curated'

    if monkey == 'Perle':
        probe_names = ['v_probe_0', 'v_probe_1']
    else:
        probe_names = ['v_probe_0']

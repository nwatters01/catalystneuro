...
addpath(genpath([homepath,'Kilosort'])) % path to kilosort folder
...
rootKS = sprintf('%s/ks2.5_output', rootZ);
if isfolder(rootKS) == 0
    mkdir(rootKS);
end
pathToYourConfigFile = [homepath, 'Kilosort/configFiles/']; % take from Github folder and put it somewhere else (together with the master_file)
chanMapFile = 'chanMap_vprobe64.mat';

run([homepath, 'Kilosort/CUDA/mexGPUall.m']);
